...
IK is a preview-only tool for rigid rotational and linear joint hierarchies, with endpoint positioning and optional single-joint target following. It never sends motor commands.
...
Select any joint group and adjust **Movement Cost** in **Inverse Kinematics**, or tap its value for exact entry. Costs range from 0.1 to 100; the default is 1. Lower costs favor movement, while higher costs discourage it. For example, wrist 1, elbow 4, and shoulder 20 favor the wrist and elbow. These are soft preferences, not locks or strict fallback priorities: costly joints can still move, including when cheaper joints reach their limits. The weighted damped least-squares solver applies costs to normalized rotational/linear increments; it does not globally minimize total motion. Coupled jaws share their motion source's cost, and editing either jaw updates that source. Costs are saved with undo/redo, and changing them stops IK and resets preview like other rig edits. Existing rigs default to equal costs and retain their previous solver behavior.

To aim the collar or head at whichever IK target is active, select the **collar/head joint itself**, enable **Follow Active IK Target**, and choose its forward direction in model-rest coordinates (a signed axis or custom vector). The follower needs a rotational axis but **no IK helper**. **Forward Direction** changes the configured direction; switching following off disables it. Put each arm's helper on the hand/gripper point you want to position. Both arms can have helpers: select either arm and use **Drag IK Target** to make that target active, and the same collar follows it. Only the active helper is solved; this does not pin the inactive arm's endpoint in world space when a shared collar moves.

The follower turns toward the target about its existing pivot/axis, compensating for its parent's pose. It may be in the arm chain or on a separate head branch, including outside the chosen chain root. The solver reserves that joint for aiming and uses the remaining joints for endpoint reach; movement cost does not override its aiming role. One hinge only controls the direction projected onto its rotation plane, not full three-dimensional gaze. Limits can prevent alignment; a target on the hinge axis preserves the current angle. Following chooses the nearest equivalent heading within the limits; a bounded joint may need a long turn across its range. The follow joint and its descendants are included in collision checks. Collision-blocked intermediate poses may not fully face the target. Settings are saved on the joint with undo/redo, independent of either arm's helper. Older helper-owned follow settings are recognized for all active targets; editing the follower replaces those older references with its joint-owned setting.

Helper points and chain roots are saved with the rig and support document undo/redo. Solved poses are temporary, like slider previews. The solver respects motor-binding rotational limits (unbound rotational joints use -180 to 180 degrees) and configured linear travel limits in millimeters. Apart from an explicitly selected follow joint, ancestors above the selected root retain their positions and independent joints on other branches are not solved. Linked jaws are one degree of freedom: targeting either jaw can solve the source travel, even when the source is a sibling, and both branches are collision checked. Chains are limited to 32 groups. Unreachable or constrained targets retain the best pose found; convergence is not guaranteed. There is no full endpoint orientation target, pole vector, or simultaneous multi-target constraint. Existing rigs without IK helpers still load unchanged.
